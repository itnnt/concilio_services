...
public class FtpGetRemoteFilesTaskletConfiguration {
...
//    }
    ...
    @Bean
    Job ftpGetRemoteFilesJob() {
        Step step = stepBuilderFactory.get("ftp get remoted files step")
                .tasklet(myFtpGetRemoteFilesTasklet())
                .build();

        return jobBuilderFactory.get("ftp get remoted files job")
                .start(step)
                .build();
    }
...
//        ftpTasklet.setLocalDirectory(new File(System.getProperty("java.io.tmpdir")));
        ftpTasklet.setLocalDirectory(new File("target/ftp-output"));
